...
export async function sayTTS(character:character,text:string) {
    try {
        ...
        switch(character.ttsMode){
            ...
            case 'doubao': {
                const cfg = character.doubaoTTSConfig
                if (!cfg?.appid || !cfg?.token || !cfg?.voiceType) {
                    throw new Error('豆包 TTS 需要填写 App ID、Access Token 和 Voice Type')
                }

                const endpoint = cfg.endpoint?.trim() || 'https://openspeech.bytedance.com/api/v1/tts'
                const encoding = cfg.encoding || 'mp3'
                const response = await globalFetch(endpoint, {
                    ...
                    body: {
                        ...
                        user: {
                            uid: cfg.uid || character.chaId || 'xiaoxianguan',
                        },
                        audio: {
                            voice_type: cfg.voiceType,
                            encoding,
                            speed_ratio: cfg.speedRatio ?? 1,
                            volume_ratio: cfg.volumeRatio ?? 1,
                            pitch_ratio: cfg.pitchRatio ?? 1,
                        },
                        request: {
                            reqid: crypto.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`,
                            text,
                            text_type: 'plain',
                            operation: 'query',
                        },
                ...
                if (response.ok && response.data?.data) {
                    await playAudio(
                        ...
                        getAudioMimeType(encoding),
                        { ttsMode: character.ttsMode ?? '', characterId: character.chaId }
                    )
                } else {
                    const detail = response.data?.message || response.data?.errmsg || response.data?.error || JSON.stringify(response.data)
