...
import { translateCharacterCardToChinese } from "./translator/characterTranslator"
...
export async function importCharacter(arg: {
    ...
    try {
        ...
        for(const f of files){
            await importCharacterProcess({
                name: f.name,
                data: f,
                translateToChinese: arg.translateToChinese,
            })
            checkCharOrder()
        }
    } catch (error) {
...
export async function importCharacterProcess<T extends boolean = false>(f:{
    ...
    if(f.name.endsWith('json')){
        ...
        if(await importCharacterCardSpec(da, undefined, 'normal', {}, null, false, f.translateToChinese)){
            let db = getDatabase()
            return db.characters.length - 1 as any
        }
        if((da.char_name || da.name) && (da.char_persona || da.description) && (da.char_greeting || da.first_mes)){
            let db = getDatabase()
            let char = convertOffSpecCards(da)
            if(f.translateToChinese){
                try {
                    char = await translateCharacterCardToChinese(char)
                } catch (error) {
                    notifyError(`角色卡翻译失败，已导入原文：${error}`)
                }
            }
    ...
    if(f.name.endsWith('charx') || f.name.endsWith('jpg') || f.name.endsWith('jpeg')){
        ...
        alertStore.set({
            type: 'wait',
            msg: language.downloadProcess.reading
        })
        ...
        await importer.done()
        let v = await importCharacterCardSpec(card, undefined, 'normal', importer.assets, lorebook, f.returnCharacter, f.translateToChinese)
    ...
    for await (const chunk of readGenerator){
        ...
        if(chunk.key.startsWith('chara-ext-asset_')){
            ...
            if(pngChunks === 0){
                alertWait(language.downloadProcess.loadedAssets.replace('{n}', readedPngChunks.toString()))
            }
            else{
                alertStore.set({
                    type: 'progress',
                    msg: language.downloadProcess.loadingAssets,
            ...
            if(metaData.usePassword){
                ...
                if(!password){
                    ...
                    try {
                        ...
                        if(await importCharacterCardSpec(charaData, img, "normal", assets, null, false, f.translateToChinese)){
                            let db = getDatabase()
                            return db.characters.length - 1
                        }
                        else{
                            throw new Error(language.downloadProcess.importError)
                        }
                    } catch (error) {
    ...
    await importCharacterCardSpec(parsed, img, "normal", assets, null, false, f.translateToChinese)
...
export async function characterURLImport() {
    ...
    try {
        if(charPath){
            alertWait(language.downloadProcess.loadingFromChub)
            const url = new URL(location.href);
        ...
        if(risuext.emotions){
            for(let i=0;i<risuext.emotions.length;i++){
                ...
                if(risuext.emotions[i][1].startsWith('__asset:')){
                    ...
                    if(!imgp){
                        throw new Error(language.downloadProcess.assetNotFound.replace('{key}', key))
                    }
        ...
        if(risuext.vits){
            ...
            for(let i=0;i<keys.length;i++){
                ...
                if(risuext.vits[key].startsWith('__asset:')){
                    ...
                    if(!imgp){
                        throw new Error(language.downloadProcess.assetNotFound.replace('{key}', rkey))
                    }
            ...
            for(let i=0;i<data.assets.length;i++){
                ...
                if(data.assets[i].uri.startsWith('__asset:')){
                    ...
                    if(b64.length < 50 * 1024 * 1024){
                        ...
                        alertError(language.downloadProcess.dataUriTooLarge)
                        continue
                    }
                }
                else{
...
export async function exportCharacterCard(char:character, type:'png'|'json'|'charx'|'charxJpeg' = 'png', arg:{
    ...
    try{
        ...
        if(!arg.writer && type !== 'json'){
            const nameExt = {
                'png': [language.downloadProcess.fileTypeImage, 'png'],
                'json': [language.downloadProcess.fileTypeJSON, 'json'],
                'charx': [language.downloadProcess.fileTypeCharX, 'charx'],
                'charxJpeg': [language.downloadProcess.fileTypeCharXJpeg, 'jpeg']
            }
        ...
        if(spec === 'v2'){
            ...
            if(card.data.extensions.risuai.emotions && card.data.extensions.risuai.emotions.length > 0){
                for(let i=0;i<card.data.extensions.risuai.emotions.length;i++){
                    onProgress(language.downloadProcess.addingEmotions, i / card.data.extensions.risuai.emotions.length * 100)
            ...
            if(card.data.extensions.risuai.additionalAssets && card.data.extensions.risuai.additionalAssets.length > 0){
                for(let i=0;i<card.data.extensions.risuai.additionalAssets.length;i++){
                    onProgress(language.downloadProcess.addingAssets, i / card.data.extensions.risuai.additionalAssets.length * 100)
            ...
            if(char.vits && char.ttsMode === 'vits'){
                ...
                for(let i=0;i<keys.length;i++){
                    onProgress(language.downloadProcess.addingVITS, i / keys.length * 100)
                    const key = keys[i]
            ...
            await sleep(10)
            onProgress(language.downloadProcess.writing, 100)
            ...
            if(card.data.assets && card.data.assets.length > 0){
                for(let i=0;i<card.data.assets.length;i++){
                    onProgress(language.downloadProcess.addingAssets, i / card.data.assets.length * 100)
...
export async function getRisuHub(arg:{
    ...
    try {
        arg.search = (arg.search + ' __shared').trim()
...
export async function downloadRisuHub(id:string, arg:{
    ...
    try {
        if(!arg.forceRedirect){
            ...
            alertStore.set({
                type: "wait",
                msg: language.downloadProcess.downloading
            })
        }
        ...
        alertError(language.downloadProcess.importError)
    }
}
