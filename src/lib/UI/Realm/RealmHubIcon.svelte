<script lang="ts">
    ...
    function normalizeDescription(desc: string | undefined) {
    ...
    function limitDescription(desc: string) {
        ...
        return `${chars.slice(0, MAX_DESCRIPTION_LENGTH).join('')}...`;
    }
    ...
    let fullDescription = $derived.by(() => {
        ...
        return normalizeDescription(
            descriptions[DBState.db.language]
            ?? descriptions.en
            ?? descriptions.xx
        );
    });

    let displayDescription = $derived(limitDescription(fullDescription));
</script>
...
<button class="bg-darkbg rounded-lg p-4 flex flex-col hover:bg-selected transition-colors relative lg:w-96 w-full items-start" onclick={onClick}>
    ...
    <div class="flex flex-col grow min-w-0">
        ...
        <span class="hub-description text-textcolor2 text-xs min-w-0 max-w-full overflow-hidden text-start" title={fullDescription}>{displayDescription}</span>
